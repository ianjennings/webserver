...
var Webserver = exports.Webserver = function(options, callback){
  ...
  self.onAny(function(data){
    var ev = 'browser::' + this.event;
    parts = this.event.split("::");
    if (parts[1] == "private") {
      self.clients[parts[0]].message(ev, data, callback);
    } else if (parts[0] !== "browser") {
      for(c in self.clients){
        self.clients[c].message(ev, data, callback);
      };
    }
  });
...
Webserver.prototype._startHTTPServer = function(callback){
    ...
    dnode(function (client, conn) {
      ...
      this.message = function (event, data, callback) {
        // we loose the connection id when emitting the event
        event = conn.id + "::" + event;
